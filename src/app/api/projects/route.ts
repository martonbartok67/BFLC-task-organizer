...
export async function POST(request: Request) {
  ...
  // Phase 3: Auto-assign creator to project
  ...
  if (assignError) {
    console.error("Failed to assign creator to project:", assignError);
    return NextResponse.json(
      { error: "Failed to set up project assignment. Please try again." },
      { status: 500 }
    );
  }
